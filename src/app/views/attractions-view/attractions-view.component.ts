...
export class AttractionsViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.attractionsService
      .getHotelAttractionsMetadata()
      .then((attractions: Attraction[]) => {
        attractions.map(attraction =>
          this.mapConfigs?.markers?.push({
        ...
        this.mapConfigs.centerLatitude =
          attractions.reduce(
            (sum, attraction) => sum + attraction.location.lat,
            0
          ) / attractions.length;
        this.mapConfigs.centerLongitude =
          attractions.reduce(
            (sum, attraction) => sum + attraction.location.lng,
            0
          ) / attractions.length;
